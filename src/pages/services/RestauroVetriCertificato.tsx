  ...
  const faqs = [
    {
      ...
      answer:
        "È un processo professionale che permette di sostituire solo il vetro esterno danneggiato del display, mantenendo intatto il pannello OLED o LCD originale Apple. Utilizziamo macchinari di laminazione professionali e adesivi OCA di alta qualità per garantire un risultato perfetto.",
    },
    {
      question: "Quali dispositivi possono essere rigenerati?",
      answer:
        "Rigeneriamo display di iPhone (dal modello 6 in poi), iPad e Apple Watch. La fattibilità dipende dalle condizioni del pannello interno: se l'OLED/LCD non presenta danni, il restauro è possibile.",
    },
    {
      question: "Il True Tone e Face ID funzioneranno ancora?",
      answer:
        "Sì, assolutamente. Il processo di rigenerazione non tocca i componenti elettronici. True Tone, Face ID, 3D Touch e tutte le funzionalità originali rimangono perfettamente operative.",
    },
    {
      question: "Quanto tempo richiede l'intervento?",
      answer:
        "Il processo di rigenerazione richiede 1-2 giorni lavorativi. Questo tempo è necessario per garantire una laminazione perfetta e test di qualità approfonditi.",
    },
    {
      question: "Cosa succede se il display si danneggia durante la rigenerazione?",
      answer:
        "Offriamo la Garanzia di Restauro Totale: se il display si danneggia irreparabilmente durante il nostro intervento, lo sostituiamo con un display di pari qualità a nostre spese. Zero rischi per te.",
    },
    {
      question: "Posso spedire il dispositivo da tutta Italia?",
      answer:
        "Sì! Accettiamo spedizioni da tutta Italia. Ti forniamo istruzioni dettagliate per l'imballaggio sicuro e un preventivo gratuito basato sulle foto che ci invii via WhatsApp.",
    },
  ];
  return (
    <>
      <Helmet>
        ...
        <meta
          name="keywords"
          content="restauro vetro iPhone, rigenerazione display, display originale Apple, riparazione vetro Assemini, GioLab, laminazione display"
        />
        ...
        <meta
          ...
          content="Restauro vetro display iPhone e iPad ad Assemini. Mantieni l'OLED originale Apple, risparmia fino al 50%."
        />
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            description:
              "Servizio professionale di rigenerazione vetro display per iPhone, iPad e Apple Watch. Mantieni la qualità originale Apple risparmiando fino al 50%.",
            provider: {
              "@type": "LocalBusiness",
            ...
            <div className="container mx-auto px-4">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                {vantaggi.map((vantaggio, index) => (
                  <Card
                    key={index}
                    className="border-0 shadow-lg hover:shadow-xl transition-shadow bg-gradient-to-br from-slate-50 to-white"
                  >
            ...
            <div className="container mx-auto px-4">
              ...
              <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
                {pricingOptions.map((option, index) => (
                  <Card
                    key={index}
                    className={`relative overflow-hidden transition-all duration-300 ${option.highlighted ? "border-2 border-giolab-blue shadow-2xl scale-105 z-10" : "border border-slate-200 hover:shadow-lg"}`}
                  >
                    {option.badge && (
                    ...
                    <CardContent className="p-6">
                      <div className="text-center mb-6">
                        <h3
                          className={`font-bold text-lg mb-1 ${option.highlighted ? "text-giolab-blue" : "text-foreground"}`}
                        >
                          {option.title}
                        </h3>
                        <p className="text-sm text-muted-foreground mb-4">{option.subtitle}</p>
                        <div
                          className={`text-3xl font-bold ${option.highlighted ? "text-giolab-blue" : "text-foreground"}`}
                        >
                          {option.price}
                        </div>
                      </div>
            ...
            <div className="container mx-auto px-4">
              <div className="max-w-3xl mx-auto">
                ...
                <Accordion type="single" collapsible className="space-y-4">
                  {faqs.map((faq, index) => (
                    <AccordionItem key={index} value={`faq-${index}`} className="border rounded-lg px-6 bg-slate-50">
